...
@FeignClient(name = "ms_produtos", url = "${icompras.pedidos.clients.produtos.url}")
public interface ProdutosClient {
    @GetMapping("/{codigo}")
    ResponseEntity<ProdutoRepresentation> obterDetalhesDoProduto(@PathVariable Long codigo);
}
